...
void main() {
  testWidgets(
      'pull-to-refresh fail while list is showing: snackbar with 重试; cards stay',
      (tester) async {
    var loads = 0;
  ...
  testWidgets(
      'feed-error-retry success calls load again, hides snackbar, shows 已是最新',
      (tester) async {
    var loads = 0;
    ...
    hang.complete(_fixtureWithUpdatedAt(_updatedA));
    await tester.pumpAndSettle();
  ...
  testWidgets('feed-error-retry success with newer stamp shows 已更新',
      (tester) async {
    var loads = 0;
    await tester.pumpWidget(
      _app(
  ...
  testWidgets('feed-error-retry failure shows the same snackbar again',
      (tester) async {
    var loads = 0;
    await tester.pumpWidget(
      _app(
