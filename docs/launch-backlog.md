...
日期：2026-09-07
现行批次：T01–T25，勾选状态见 `docs/delivery.md`。
接口：`docs/api.md`。部署：`docs/deploy.md`。

**未上线。** 缺 Cloudflare 配置、Access 允许身份、`XAI_API_KEY`。即时推送默认关闭。T24 七天观察未开始，不得标记口味达标。

旧 P0/P1 文本留作历史对照。规则以 T01–T25 为准：死亡/灾难不再自动升格，首页是精选配额 18/9/3，GitHub 是开源发现，热搜只当线索。

---

历史快照（2026-08-25）：对照 https://toolazytoname.github.io/AquaSight/ ，`events.json` 更新于 `2026-08-25T08:00:10Z`。
