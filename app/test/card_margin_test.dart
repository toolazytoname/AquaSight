...
void main() {
  testWidgets(
      'breaking and normal cards share vertical margin 4 and elevation 0',
      (tester) async {
    await _pumpBoth(tester);

    final expectedMargin = const EdgeInsets.symmetric(vertical: 4);

    final breaking = tester.widget<Card>(find.byKey(_breakingCardKey));
    expect(breaking.margin, expectedMargin);
    expect(breaking.elevation, 0);

    final normal = tester.widget<Card>(find.byKey(_normalCardKey));
    expect(normal.margin, expectedMargin);
