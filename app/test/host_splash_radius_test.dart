...
void main() {
  testWidgets(
      'event-card-same-day-breaking-host nearest InkWell customBorder is radius 8; splash tokens stay; no onTap',
      (tester) async {
    _setDefaultSurface(tester);
    await _pumpBreaking(tester);

    final hostFinder = find.byKey(_breakingHostKey);
    expect(hostFinder, findsOneWidget);
    expect(tester.widget(hostFinder), isA<Text>());
