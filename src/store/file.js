...
export async function loadFileStore(path) {
  ...
  async function withWriteLock(fn) {
    ...
    for (let i = 0; i < 50; i++) {
      try {
        ...
        if (e && e.code === "EEXIST") {
          try {
            const st = await stat(writeLock);
            if (Date.now() - st.mtimeMs > 30000) await rm(writeLock, { force: true });
          } catch {
            // gone
          }
          await sleep(15);
  ...
  async function reload() {
    ...
    await mem.importAll(next);
  }

  async function saveUnlocked() {
    await mkdir(dirname(path), { recursive: true });
    const json = JSON.stringify(await mem.exportAll(), null, 2) + "\n";
  ...
  for (const name of Object.keys(mem)) {
    if (typeof mem[name] !== "function") continue;
    if (name === "articleEventMap") continue;
    store[name] = async (...args) => {
      return withWriteLock(async () => {
        await reload();
        const result = await mem[name](...args);
