...
void main() {
  testWidgets(
      'breaking and normal cards share transparent shadow, tint, elevation 0',
      (tester) async {
    await _pumpBoth(tester);

    final breaking = tester.widget<Card>(find.byKey(_breakingCardKey));
    expect(breaking.shadowColor, Colors.transparent);
    expect(breaking.surfaceTintColor, Colors.transparent);
    expect(breaking.elevation, 0);

    final normal = tester.widget<Card>(find.byKey(_normalCardKey));
    expect(normal.shadowColor, Colors.transparent);
    expect(normal.surfaceTintColor, Colors.transparent);
